...
<script>
    function FrmUpdate(e, spin, submit, myData) {
        e.preventDefault();
        $(spin).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
    ...
    $(function () {
        $('#FrmUpdateMember').submit(function (e) {
            FrmUpdate(e, '.myLoad1', '#mySubmit1', $(this).serialize())
        });
    });

    $(function () {
        $('#FrmUpdateStatus').submit(function (e) {
            FrmUpdate(e, '.myLoad2', '#mySubmit2', $(this).serialize())
        });
    });

    $(function () {
        $('#FrmUpdateDesc').submit(function (e) {
            FrmUpdate(e, '.myLoad3', '#mySubmit3', $(this).serialize())
        });
    });
</script>
...
<script>
    $('#orders1, #orders2 , #orders3').sortable({
        ...
        update: function () {
            ...
            $.ajax({
                method: 'post',
                url: '{{route('admin.task.sortable',$project->id)}}',
    ...
    $('.FrmComment').submit(function (e) {
        ...
        $.ajax({
            ...
            error: function (response) {
                ...
                $('.mySubmit1').prop('disabled', false);
                $.each(response.responseJSON.errors, function (key, value) {
                    let input2 = $('.FrmComment').find('textarea[name^=' + key + ']');
                    input2.parents('.form-group').append(`<small class="row text-danger ml-1">${value}</small>`);
                });
            }
        });
